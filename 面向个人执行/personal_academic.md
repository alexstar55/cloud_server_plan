...
**补充结论**：Phase 1/Phase 2 与“4D 自动标注”可自然关联，但本月只做**延伸验证**（伪标签样例与一致性对比），不新增第三条主线。
...
## 8) 与 4D 自动标注的延伸关系（本月可选）
- 方法关联：
  - Phase 1 可提供“动作意图/轨迹”类自动标签样例。
  - Phase 2 可提供“检测/车道/轨迹”伪标签，适配 4D 时序一致性验证。
- 最小交付：
  - 1 组伪标签样例（含可视化）；
  - 1 页一致性对比（跨帧连续性 + 几何合理性 + 误差归因）。
- 边界条件：
  - 不改本月主验收（Phase 1/2）；
  - 不引入重型新框架复现任务；
  - 若影响主线进度，立即冻结延伸分支。
